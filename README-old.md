...
## 基于 RuoYi-Vue-Plus 的融合改造方案（优化版）

### 一、项目整体架构设计

#### 1.1 技术栈整合方案

**后端技术栈（基于 RuoYi-Vue-Plus 扩展）**
...
| 分布式任务 | SnailJob | 1.9.0 | 原框架已集成 |
| 分布式锁 | Lock4j + Redisson | 2.2.7/3.52.0 | 原框架已集成 |
| 对象存储 | Minio/AWS S3 | 2.28.22 | 原框架已集成 |
...
```
voicetext-weaver/
├── voicetext-admin/                 # 系统启动模块(原ruoyi-admin)
├── voicetext-common/                 # 通用工具模块(原ruoyi-common)
...
│       ├── src/main/java/com/voicetext/audio/
│       │   ├── core/                 # 音频核心处理
│       │   │   ├── AudioProcessor.java
...
│       │   │   ├── EditOperationStack.java  # 操作栈
│       │   │   └── dto/
│       │   └── dsp/                  # 数字信号处理
│       │       ├── AudioAnalyzer.java
│       │       └── VoiceActivityDetector.java
│       └── pom.xml
├── voicetext-modules/                 # 业务模块(原ruoyi-modules)
│   ├── voicetext-system/              # 系统管理
│   ├── voicetext-tenant-resource/     # 【新增】租户资源池管理
│   │   ├── src/main/java/com/voicetext/tenantresource/
│   │   │   ├── controller/
│   │   │   │   ├── AudioResourceController.java
│   │   │   │   ├── TextResourceController.java
│   │   │   │   └── ResourceShareController.java
│   │   │   ├── service/
│   │   │   │   ├── IResourcePoolService.java
│   │   │   │   ├── IResourcePermissionService.java
│   │   │   │   └── impl/
│   │   │   ├── mapper/
│   │   │   │   ├── TenantResourceMapper.java
│   │   │   │   └── ResourceShareMapper.java
│   │   │   ├── domain/
│   │   │   │   ├── TenantResource.java
│   │   │   │   ├── ResourceShare.java
│   │   │   │   ├── ResourceAccessLog.java  # 【新增】
│   │   │   │   └── vo/
│   │   │   └── config/
│   │   └── pom.xml
│   └── voicetext-workflow/            # 工作流(音频审核流程)
└── voicetext-extend/                  # 扩展组件(原ruoyi-extend)
    └── voicetext-job/                  # 定时任务
```
...
**1.3.8 资源访问日志表 (resource_access_log)**
```sql
...
**3.1.2 查询训练状态**
```http
GET /api/voice/train/status/5001
...
GET /api/voice/clone/result/6001
...
**3.2.2 添加音频到项目**
```http
POST /api/project/1001/add-audio
...
GET /api/project/merge/result/7001
...
```sql
ALTER TABLE sys_tenant ADD COLUMN voice_model_quota INT DEFAULT 5;
ALTER TABLE sys_tenant ADD COLUMN voice_model_used INT DEFAULT 0;
ALTER TABLE sys_tenant ADD COLUMN project_quota INT DEFAULT 50;
ALTER TABLE sys_tenant ADD COLUMN project_used INT DEFAULT 0;
ALTER TABLE sys_tenant ADD COLUMN clone_quota INT DEFAULT 1000;
ALTER TABLE sys_tenant ADD COLUMN clone_used INT DEFAULT 0;
ALTER TABLE sys_tenant ADD COLUMN merge_quota INT DEFAULT 500;
ALTER TABLE sys_tenant ADD COLUMN merge_used INT DEFAULT 0;
```
